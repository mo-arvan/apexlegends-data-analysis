...
class DynamicFilters:
    ...
    def __init__(self, df, filters_dict, filters_name='filters', order_dict=None, default_filters=None):
        """
        ...
        ----------
            df : DataFrame
                The dataframe on which filters are applied.
            filters_dict : dict of filters
                Dictionary with column names as keys and their name aliases as values.
        ...
        for key, value in filters_dict.items():
            self.filters[key] = [] if key not in default_filters else default_filters[key]
    ...
    def display_filters(self, location=None, num_columns=0, gap="small"):
        ...
        for filter_name in st.session_state[self.filters_name].keys():
            filter_displayed_name = self.filters_dict[filter_name]
            filtered_df = self.filter_df(filter_name)
            options = filtered_df[filter_name].unique().tolist()

            if filter_name in self.order_dict:
                options = sorted(options, key=lambda x: self.order_dict[filter_name].index(x), reverse=False)
